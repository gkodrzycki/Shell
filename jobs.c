...
static void sigchld_handler(int sig) {
  ...
  // Przechodzimy po wszystkich możliwych procesach
  for (int i = 0; i < njobmax; i++) {
    for (int j = 0; j < jobs[i].nproc; j++) {
      pid =
        waitpid(jobs[i].proc[j].pid, &status, WNOHANG | WUNTRACED | WCONTINUED);

      // Teraz rozważamy przypadki jeśli czekaliśmy na dany proces
...
#ifdef STUDENT
  // Chcemy zapisać exitcode i usunąć joba jesli już się skończył
  if (state == FINISHED) {
...
bool resumejob(int j, int bg, sigset_t *mask) {
  ...
  // jeśli nie powinien być w bg to przesuwamy fo fg
  // używamy while żeby uniemożliwić racecondy
  if (!bg) {
...
#ifdef STUDENT
  // Zabijamy i wysyłamy SIGCONTA żeby go obudzić jakby coś mu nie docierało
...
int monitorjob(sigset_t *mask) {
  ...
  // Dajmy joba do bg jeśli jest zastopowany
  if (state == STOPPED) {
    int jobnumber = allocjob();
    Tcgetattr(tty_fd, &jobs[jobnumber].tmodes);
    movejob(0, jobnumber);
  }
